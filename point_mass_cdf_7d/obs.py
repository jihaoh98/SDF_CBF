...
class Cdf_Obs:
    ...
    def get_current_state(self):
        ...
        current_state = np.array([self.state[0], self.state[1]])
        return current_state


class Circle_Obs:
    def __init__(self, index, radius, center, vel=np.zeros((7,)), goal=np.zeros((7, 1)), mode='static',
                 **kwargs) -> None:
    ...
    def get_current_state(self):
        """ return the obstacle's position and velocity, as well as radius """
        current_state = np.array([self.state[0], self.state[1], self.state[2], self.state[3], self.state[4],
                                  self.state[5], self.state[6], self.radius])
        return current_state

    def get_current_vel(self):
        current_vel = np.array([self.vel[0], self.vel[1], self.vel[2], self.vel[3], self.vel[4], self.vel[5],
                                self.vel[6]])
        return current_vel
