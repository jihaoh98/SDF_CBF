...
class Circle:
    ...
    def create_patch(self, color='k'):
        ...
        radius = self.radius
        circle = patches.Circle(center, radius, linewidth=1, edgecolor=color, facecolor='none', hatch='///', label=self.label)
        return circle
...
class Box:
    ...
    def sample_surface(self, N):
        ...
        x = torch.linspace(-self.w / 2, self.w / 2, steps=N_per_edge + 1, device=self.device)[:-1]  # Avoid duplicate at corner
    ...
    def create_patch(self,color='r'):
        ...
        if self.label is None:
            rect = patches.Rectangle(center-[w/2.0, h/2.0] , w, h, linewidth=1, edgecolor=color, facecolor='none', hatch='///')
        else:
            rect = patches.Rectangle(center-[w/2.0, h/2.0] , w, h, linewidth=1, edgecolor=color, facecolor='none', hatch='///', label=self.label)
        return rect
