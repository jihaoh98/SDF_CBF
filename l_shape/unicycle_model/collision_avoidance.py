...
class Collision_Avoidance:
    ...
    def show_both_cbf(self):
        """ show both cbf """
        t = np.arange(0, self.terminal_time / 10, self.step_time)
        plt.plot(t, self.obs_cbf_t[0][:, 0:self.terminal_time].reshape(self.terminal_time), linewidth=2, color='grey', label='polytopic-shaped obstacle1')
        plt.plot(t, self.obs_cbf_t[1][:, 0:self.terminal_time].reshape(self.terminal_time), linewidth=2, color='blue', linestyle='dashed', label='polytopic-shaped obstacle2')
