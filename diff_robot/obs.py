...
class Polytopic_Obs:
    def __init__(self, index, vertex=None, vel=np.zeros((2, )), goal=np.zeros((2, 1)),
                 obs_model='static', **kwargs) -> None:
        ...
        if vertex is not None:
            # in shape (n, 2)
            self.init_vertexes = np.array(vertex)
    ...
    def get_center_position(self):
        ...
        # shape in (2, )
        self.position = np.array([center_x, center_y])

    def get_vertex_vector(self):
        """ get the vertex vetcor """
    ...
    def update_vertexes(self):
        """ update the vertexes when the robot moves"""
        for i in range(self.ver_num):
    ...
    def get_current_vertexes(self, cur_position):
        """ get the vertexes according the cur position """
    ...
    def arrive_destination(self):
        """ determine if the robot arrive its goal position """
    ...
    def get_current_state(self):
        """ return the robot's position and velocity """
...
if __name__ == '__main__':
    ...
    obs_params = config['obstacle']

    # init
    test_target = Polytopic_Obs(0, obs_params['obs_vertexes'], obs_params['obs_vel'], obs_model=obs_params['obs_model'])

    print(test_target.position)    
    print(test_target.get_current_state())

    test_target.move_forward(0.1)
    print(test_target.get_current_state())
    print(test_target.inside_obstacle(np.array([[1.1], [1.5]])))
    print(test_target.inside_obstacle(np.array([[1.5], [1.5]])))
