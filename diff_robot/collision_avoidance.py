...
class Collision_Avoidance:
    def __init__(self) -> None:
        ...
        # in shape (N, 2)
        # self.robot_vertexes = np.array(robot_params['robot_vertex'])
    ...
    def collision_avoidance(self):
        ...
        while np.linalg.norm(self.robot_cur_state[0:2] - self.robot_target_state[0:2]) >= self.destination_margin and t - self.time_steps < 0.0:
            ...
            u, clf1, clf2, slack1, slack2, feas = self.cbf_qp.cbf_clf_qp(self.robot_cur_state, obs_vertexes_list, self.obs_states_list, add_clf=False, u_ref=u_ref)
            process_time.append(time.time() - start_time)
            if not feas:
                print('This problem is infeasible, we can not get a feasible solution!')
                break
    ...
    def render(self):
        ...
        # robot and the arrow
        cur_vertexes = self.robot.get_vertexes(self.robot_init_state)
        self.robot_body = mpatches.Polygon(cur_vertexes, edgecolor='silver', facecolor=None)
